...
cd /d %inprout%
tar -zcf "%outprout%\ajour_backup_%date:~10,4%-%date:~4,2%-%date:~7,2%_%time:~0,2%-%time:~3,2%-%time:~6,2%.tar.xz" Interface WTF
echo erreur lvl %errorlevel%
if %errorlevel% ==0 (del "%outprout%%Dossier%" & start "" "%USERPROFILE%\AppData\Local\Programs\wowup-cf\WowUp-CF.exe" & exit) else (echo le backup s est mal passe & goto UnknowError)
